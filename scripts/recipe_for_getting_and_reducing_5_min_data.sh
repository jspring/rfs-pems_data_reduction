#!/bin/bash
# This script creates PEMs data files containing 5 minute data files containing flow, occupancy, and speed 
# data.  Each file is named with the convention IIIIIII_MM_DD_YYYY.txt, where IIIIIII = station ID, MM=month,
# SS=day, and YYYY=year
#
# It uses 5-minute data files and metadata files, both of which are downloaded from the PeMS website.
...
do
	...
	y=`echo $NOCOMMAS | cut -d ' ' -f 14,15,16,19,20,21,24,25,26,29,30,31,15,16,19,20,21,24,25,26,29,30,31,15,16,19,20,21,24,25,26,29,30,31,34,35,36,39,40,41,44,45,46,49,50,51`
